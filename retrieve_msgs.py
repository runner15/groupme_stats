	...
	while (curCount < totalCount):
		...
		for msg in msgs:
			...
			if user not in users:
				users[user] = []
			if csv_file:
				wr.writerow([group_name.decode("utf-8"), created_at, user, text, str(likes)])
...
def main(retrieve_all, direct_msgs, group_name, csv_file, overwrite):
	...
	if retrieve_all:
		...
		if group_name not in groups:
			print(group_name + " not found. Here are the list of groups:")
			all_groups = getGroupNames(groups)
			print(all_groups)
			print(type(list(all_groups)[0]))
